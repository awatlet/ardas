...
const int NUMBER_OF_COUNTERS = 2; // 32 bits counters - WARNING Change BYTES_VAL_T type to unsigned long if more than 2 counters 
...
RTC_DS1307 RTC;
//RTC_DS1307::Frequencies RTC_freq;
...
void rtc_interrupt()
{
  ...
  if (pulse_counter % 2048 == 0){  // freq/2 but does not work with 1 Hz
...
void loop(){
  if(read_counter_register){
    ...
    n += 1;
    if (n % (2*sampling_rate) == 0) {
          DateTime now = RTC.now();
          DateTime tic = now.unixtime() - sampling_rate/2;
