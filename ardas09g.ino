...
#include <SD.h> // WARNING : REMOVED FOR DEBUG
#include <SPI.h> // WARNING : REMOVED FOR DEBUG
...
File data_file; // WARNING : REMOVED FOR DEBUG
...
void download_record(){
    ...
    data_file.seek(p); // WARNING : REMOVED FOR DEBUG
...
void setup()
{
    ...
    if (!SD.begin(cs_pin)) {  // WARNING : REMOVED FOR DEBUG
    ...
    data_file.write(record,30); // WARNING : REMOVED FOR DEBUG
    data_file.flush(); // WARNING : REMOVED FOR DEBUG
    free_ram = freeRam(); // TODO : Comment this
    Serial.println(free_ram);  // TODO : Comment this
        ...
        if( first_character == '-') {
            ...
            if ((!download_flag) && (!quiet)){
                if (command == "HE") {
                    ...
                    if (data_file){  // WARNING : REMOVED FOR DEBUG
                        data_file.close();
                        SD.remove(DATA_FILE); 
                    }
                    software_reset();
                    ...
                    data_file.flush(); // WARNING : REMOVED FOR DEBUG
                    data_file.close(); // WARNING : REMOVED FOR DEBUG
